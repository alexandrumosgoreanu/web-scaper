...
if __name__ == '__main__':
    ...
    # domains_list = ["melatee.com"]
    chunks = list(divide_chunks(domains_list, 42))                 # dividing the domains into (somehwat) equal chunks to be passed to a thread
    ...
    with ThreadPoolExecutor(max_workers=cpu_cores*2) as executor:     # starting a thread pool with 2xnumber of threads as there are physical ones
        # executor.map(scrape, chunks, chunksize=2)
        futures = [executor.submit(scrape, chunk) for chunk in chunks]
